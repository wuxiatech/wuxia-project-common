...
public class BaseUtil {
     ...
     * 有效期30日
     */
    public final static int expTime = 10800;

    private static CacheClient cacheClient;

    private final static String CACHE_NAME_SPACE = "baiducachespace";
    static {
        if (properties == null) {
            properties = PropertiesUtils.loadProperties("classpath*:baidu.config.properties");
        }
        // CACHED地址
        ...
        if (StringUtil.isBlank(cachedAddr) || StringUtil.isBlank(cacheImpl)) {
            logger.error("请检查baidu.config.properties的cached.addr及cache.impl");
            throw new RuntimeException("请检查baidu.config.properties的cached.addr及cache.impl");
        }
        try {
    ...
    protected static void cleanCache() {
        logger.info("清除百度api缓存，namespace：{}", CACHE_NAME_SPACE);
