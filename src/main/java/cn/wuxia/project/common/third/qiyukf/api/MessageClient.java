...
import cn.wuxia.project.common.third.qiyukf.session.constant.OpenApiTags;
...
import cn.wuxia.project.common.third.qiyukf.session.model.QiyuMessage;
import cn.wuxia.project.common.third.qiyukf.session.model.QiyuSession;
import cn.wuxia.project.common.third.qiyukf.session.model.QueryQueueResult;
import cn.wuxia.project.common.third.qiyukf.session.util.FileUtil;
import cn.wuxia.project.common.third.qiyukf.session.util.HttpClientPool;
import cn.wuxia.project.common.third.qiyukf.session.util.MD5;
import cn.wuxia.project.common.third.qiyukf.session.util.MediaUtil;
import cn.wuxia.project.common.third.qiyukf.session.util.QiyuPushCheckSum;
...
public class MessageClient {
     ...
     */
    public MessageClient(String appKey, String appSecret) {
        this.appKey = appKey;
    ...
    public ApplyStaffResult applyStaff(ApplyStaffInfo staffInfo) throws IOException {
        ...
        if (res != null) {
            ...
            if (result.getCode() == 200) {
                result.setSession(JSONObject.toJavaObject(res, QiyuSession.class));
     ...
     * @throws IOException
     */
    public CommonResult sendAudioMessage(String fromUid, String path, long duration)
            throws IOException, LineUnavailableException, UnsupportedAudioFileException, NoSuchAlgorithmException {
        String md5 = FileUtil.getMd5(path).toLowerCase();
        long size = FileUtil.getSize(path);
        String url = uploadFile(path, md5);
    ...
    /**
     * 转发微信的消息给七鱼。
     * @param inMessage 将微信发过来的消息解密之后
     * @return 返回发送消息的结果。目前只支持文本，语音和图片，其他格式直接返回null
     * @throws IOException 从微信下载文件失败，或者是发给七鱼时失败
     */
    public CommonResult send(InMessage inMessage) throws IOException {

        switch (inMessage.getMsgType()) {
            case text:
                return sendTextMessage(inMessage.getUserid(), new String(inMessage.getContent()));
            case image:
                String md5 = MD5.md5(inMessage.getContent());
                String base64 = Base64.encodeBase64String(inMessage.getContent());
                String url = sendFile(md5, base64);

                int[] size = MediaUtil.querySize(new ByteArrayInputStream(inMessage.getContent()));
                return sendImageMessage(inMessage.getUserid(), url, md5, inMessage.getContent().length, size[0], size[1]);
            case voice:
                md5 = MD5.md5(inMessage.getContent());
                base64 = Base64.encodeBase64String(inMessage.getContent());
                url = sendFile(md5, base64);

                String format = inMessage.getAddition().get("Format");
                long duration;
                if ("amr".equalsIgnoreCase(format)) {
                    duration = MediaUtil.queryAmrDuration(inMessage.getContent());
                } else {
                    duration = MediaUtil.queryAudioDuration(new ByteArrayInputStream(inMessage.getContent()));
                }
                return sendAudioMessage(inMessage.getUserid(), url, md5, inMessage.getContent().length, duration);
            default:
    ...
    public static void main(String[] args) {
        ...
        MessageClient client = new MessageClient("23265109a6de706423539e0f6a6fb820", "B280D73CCF0648D1828305F73E9A3C47");
